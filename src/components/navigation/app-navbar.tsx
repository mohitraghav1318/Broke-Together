  ...
  return (
    <header className="sticky top-0 z-20 border-b border-emerald-100 bg-white/95 backdrop-blur">
      <nav className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link
          className="flex items-center gap-3 text-base font-bold text-zinc-900"
          href="/"
          onClick={closeMenu}
        >
        ...
        <div className="hidden items-center gap-1 md:flex">
          ...
          {user ? (
            <>
              ...
              <button
                className="rounded-md border border-emerald-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-800 transition-colors hover:bg-emerald-50 hover:text-emerald-800"
        ...
        <button
          ...
          className="grid size-10 place-items-center rounded-md border border-emerald-200 bg-white text-emerald-800 md:hidden"
          onClick={() => setIsMenuOpen((current) => !current)}
          type="button"
        >
          <span className="grid gap-1.5">
            <span className="block h-0.5 w-5 rounded bg-emerald-800" />
            <span className="block h-0.5 w-5 rounded bg-emerald-800" />
            <span className="block h-0.5 w-5 rounded bg-emerald-800" />
          </span>
        </button>
          ...
          <div className="mx-auto grid max-w-6xl gap-2">
            ...
            {user ? (
              <>
                ...
                <button
                  className="rounded-md border border-emerald-200 bg-white px-3 py-3 text-left text-sm font-semibold text-zinc-800 transition-colors hover:bg-emerald-50 hover:text-emerald-800"
                  onClick={handleSignOut}
                  type="button"
                >
                  Sign out
